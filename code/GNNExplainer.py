...
for data in dataset:
    ...
    mask.append([node_feat_mask.numpy(), edge_mask.numpy(), data.y.numpy()])  #这里把两个mask存起来，后续用
    sort_edge.append(np.argsort(edge_mask.numpy()))  #这里是根据边的mask给边排序
...
k=0
for mask_ in mask:
    em = mask_[1]  #这个是边的mask，根据节点之间相互作用，给每个边打分
    fm = mask_[0]  #这个是特征mask，根据特征的有用程度的打分
